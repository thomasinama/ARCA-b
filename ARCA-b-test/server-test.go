...
func main() {
    ...
    getGrokResponse := func(question string) (string, error) {
        if grokKey == "" {
        ...
        if err != nil {
            return "", fmt.Errorf("errore nella creazione della richiesta a Grok: %v", err)
        }
        ...
        if err != nil {
            return "", fmt.Errorf("errore con Grok: %v", err)
        }
        ...
        if err := json.NewDecoder(grokResp.Body).Decode(&grokResult); err != nil {
            return "", fmt.Errorf("errore nel parsing di Grok: %v", err)
        }
        if len(grokResult.Choices) == 0 {
            return "", fmt.Errorf("nessuna risposta valida da Grok")
        }
...
<body>
    ...
    <script>
        ...
        function addMessage(text, isUser, style, rawResponses) {
            ...
            if (!isUser && rawResponses) {
                console.log("Raw responses received:", rawResponses); // Log per debug
                ...
                console.log("No raw responses provided for this message.");
            }
        ...
        async function sendMessage() {
            ...
            try {
                ...
                console.log("Response received:", answer[0]); // Log per debug
                ...
                console.error("Errore durante la richiesta:", error); // Log per debug
                removeTypingIndicator();
...
`)
    ...
    http.HandleFunc("/ask", func(w http.ResponseWriter, r *http.Request) {
        ...
        synthesisPrompt := fmt.Sprintf(
            "Synthesize the following answers into a single, comprehensive response. Avoid bias, censorship, and propaganda. Provide a clear and concise answer in %s style:\n\nOpenAI: %s\n\nDeepSeek: %s\n\nGemini: %s",
            style, openAIAnswer, deepSeekAnswer, geminiAnswer,
        )
        synthesizedAnswer, err = getGrokResponse(synthesisPrompt)
        if err != nil {
